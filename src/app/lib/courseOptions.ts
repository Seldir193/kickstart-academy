    ...
    items: [
      {
        mode: "subtype",
        category: "RentACoach",
        value: "RentACoach_Generic",
        label: "Rent-a-Coach",
      },
      {
        mode: "subtype",
        category: "ClubPrograms",
        value: "ClubProgram_Generic",
        label: "Training Camps",
      },
      {
        mode: "subtype",
        category: "ClubPrograms",
        value: "CoachEducation",
...
const isWeekly = (o: OfferLike) =>
  o?.category === "Weekly" ||
  o?.type === "Foerdertraining" ||
  o?.type === "Kindergarten";

const isHoliday = (o: OfferLike) =>
  o?.category === "Holiday" || o?.type === "Camp";
...
const isCoachEducation = (o: OfferLike) => {
  ...
  if (cat === "clubprograms" && /\bcoach\s*education\b/.test(title))
    return true;
  return false;
};
...
const isPlainFoerdertraining = (o: OfferLike) =>
  o?.type === "Foerdertraining" &&
  !/torwart|keeper|athletik|athletic/i.test(o?.sub_type || "");
...
export function offerMatchesCourse(
  ...
  if (opt.mode === "type") {
    switch (opt.value) {
      ...
      case "Foerdertraining":
        return (
          isWeekly(offer) &&
          isPlainFoerdertraining(offer) &&
          !isRentACoach(offer) &&
          !isCoachEducation(offer)
        );
...
export function courseValueFromBooking(
  ...
  const snap = String(booking?.offerType ?? "").trim();
  if (snap) return snap;

  const oid = String(booking?.offerId ?? "").trim();
  const offer = oid ? offersById.get(oid) : null;
  if (!offer) return "";

  const st = String(offer?.sub_type ?? "").trim();
  if (st) return st;

  const t = String(offer?.type ?? "").trim();
  return t || "";
}
