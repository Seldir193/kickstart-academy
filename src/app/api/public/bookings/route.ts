  ...
  try {
    ...
    const r = await fetch(url, {
      ...
      headers: {
        ...
        "User-Agent":
          req.headers.get("user-agent") ?? "kickstart-academy/next-proxy",
      },
    ...
    if (process.env.NODE_ENV !== "production") {
      console.log("[/api/public/bookings] upstream", r.status, {
        ...
        hasMeta: !!body.meta,
        upstreamPayloadKeys:
          typeof payload === "object" ? Object.keys(payload) : typeof payload,
      });
    }
    ...
    console.error(
      "[/api/public/bookings] upstream error:",
      err?.message || err,
    );
    return NextResponse.json(
      {
