...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isAllowedForPurpose(purpose, mime)) return rejectUpload("unsupported_media_type", 415);
...
//   return `${origin}/api/admin/upload/${encodeURIComponent(fileName)}`;
// }
...
//       wantedName
//         .replace(/[^\w.\-]+/g, "")
//         .replace(/\.(png|jpe?g|webp|gif)$/i, "") || "news";
