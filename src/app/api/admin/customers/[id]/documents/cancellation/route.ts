...
async function forward(req: NextRequest, { params }: { params: { id: string; bid: string } }) {
  ...
  const url = `${apiBase()}/customers/${encodeURIComponent(params.id)}/bookings/${encodeURIComponent(params.bid)}/documents/cancellation`;
  ...
  return new NextResponse(body, {
    ...
    headers: {
      ...
      'Content-Disposition': r.headers.get('content-disposition') || 'inline; filename="Kuendigungsbestaetigung.pdf"',
    },
  });
}
