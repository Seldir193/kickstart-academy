...
//       {
//         ok: false,
//         error: "Server misconfigured: NEXT_BACKEND_API_BASE is missing",
//       },
//       { status: 500 },
//     );
//   }
...
//       req.nextUrl.searchParams.get("sort") || "issuedAt:desc",
//     );
...
//           {
//             ok: false,
//             error: `Upstream customers failed (${r.status})`,
...
//           : String(b[sortField] || "").localeCompare(
//               String(a[sortField] || ""),
//             );
...
//     await Promise.all(
//       Array.from(toFetchBookings).map(async (bid) => {
...
//     await Promise.all(
//       Array.from(toFetchCustomers).map(async (cid) => {
...
//       const list: AnyRec[] = Array.isArray(customer?.bookings)
//         ? customer!.bookings
//         : [];
...
//         d.customerName,
//       );
//       const custAddr1 = pick(
//         [customer?.address?.street, customer?.address?.houseNo]
//           .filter(Boolean)
//           .join(" "),
//         [customer?.parent?.address?.street, customer?.parent?.address?.houseNo]
//           .filter(Boolean)
//           .join(" "),
//         d.customerAddressLine1,
//       );
//       const custZip = pick(
//         customer?.address?.zip,
//         customer?.parent?.address?.zip,
//         d.customerZip,
//       );
//       const custCity = pick(
//         customer?.address?.city,
//         customer?.parent?.address?.city,
//         d.customerCity,
//       );
...
//       const title =
//         d.title ||
//         pick(d.offerTitle, d.offerType, booking.offerTitle, booking.type);
...
//       const refInvoiceNo = normalizeInvoiceNo(
//         pick(d.refInvoiceNo, bref?.refInvoiceNo),
//       );
...
//         refInvoiceNo,
//         fmtDEDate(d.refInvoiceDate),
//         d.cancellationNo || "",
//         d.stornoNo || "",
//         d.stornoAmount != null && d.stornoAmount !== ""
//           ? String(d.stornoAmount)
//           : "",
//       ].map(escCSV);
...
//         gross.toFixed(2),
//         paymentTerms,
//         fmtDEDate(paymentDueDate),
//         paymentStatus,
//         fmtDEDate(paymentDate),
//         originalInvoiceNumber,
//         creditNoteNumber,
//         notes,
//         pdfUrlExtra,
//         costCenter,
//         fmtDEDate(serviceFrom),
//         fmtDEDate(serviceTo),
//       ].map(escCSV);
