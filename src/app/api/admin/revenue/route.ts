  ...
  try {
    ...
    if (token) {
      try {
        ...
        /* ignore, auf Fallbacks zurückfallen */
      }
    }

    // 2) Fallback-Cookies (aus Sign-up/Altlogik)
    const pidCookie =
...
export async function GET(req: Request) {
  try {
    ...
    const upstream = `${apiBase()}/admin/revenue${getSearch(req)}`;
    ...
    return NextResponse.json(
      { ok: false, error: 'Revenue proxy failed', detail: String(e?.message || e) },
      { status: 500 }
    );
  }
}
