...
export default function AdminLoginForm(props: Props) {
  return (
    <section className="admin-auth" aria-busy={props.loading}>
      <div className="auth-card">
        <h1 className="auth-title">Provider Login</h1>
        <p className="auth-subtitle">
          Please enter your email and password (internal use only).
        </p>
        <AuthFormError error={props.formError} />
        <form className="auth-form" onSubmit={props.onSubmit} noValidate>
          <fieldset disabled={props.loading} className="auth-fieldset">
            <LoginFields {...props} />
            <LoginLinks />
            <LoginActions
...
function LoginActions({
  loading,
  onCreateAccount,
}: Pick<Props, "loading" | "onCreateAccount">) {
  return (
