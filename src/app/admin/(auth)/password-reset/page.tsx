...
export default function PasswordResetPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (token) {
        router.push(
          `/admin/new-password?token=${encodeURIComponent(token)}&email=${encodeURIComponent(em)}`,
        );
        return;
      }
      ...
      <div className="contact-form">
        ...
        {!sent ? (
          <>
            ...
            {error && (
              <div className="error mt-3" aria-live="polite">
                {error}
              </div>
            )}
          ...
          <>
            <p className="ok mt-3">
              Wenn ein Konto mit <strong>{email.trim()}</strong> existiert,
              haben wir dir einen Link geschickt. Bitte prüfe dein Postfach (und
              den Spam-Ordner).
            </p>
            <div className="actions">
              <button
