...
function buildSignupState(ctx: SignupContext) {
  ...
  return {
    ...
    blurField,
    disabled,
    onSubmit: createSubmitHandler(ctx),
    setField: createSetField(ctx.setForm, ctx.setErrors),
  };
}

function useSignupEmailParam(
  params: ReadonlyURLSearchParams,
...
function updateSignupForm<K extends keyof SignupForm>(
  previous: SignupForm,
  key: K,
  value: SignupForm[K],
  setErrors: Dispatch<SetStateAction<SignupErrors>>,
...
async function submitSignup(ctx: SignupContext) {
  ...
  try {
    await handleSignupResponse(
      ctx,
      await requestSignup(buildSignupPayload(ctx.form)),
    );
  } catch {
...
async function handleSignupResponse(ctx: SignupContext, response: Response) {
  ...
  if (!isSignupSuccess(response, data))
    return handleSignupError(ctx, data, response);
  ctx.setStatus("done");
  ctx.router.replace(
    `/admin/login?email=${encodeURIComponent(ctx.form.email.trim().toLowerCase())}`,
  );
}
