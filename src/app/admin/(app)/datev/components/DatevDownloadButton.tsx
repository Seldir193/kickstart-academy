...
function DownloadLabel({
  loading,
  t,
}: Pick<DatevExportModel, "loading"> & { t: TFunction }) {
  const key = loading
    ? "common.admin.datev.exportRunning"
    : "common.admin.datev.downloadZip";
...
export default function DatevDownloadButton({ model, t }: Props) {
  return (
    <button
      ...
      onClick={model.runExport}
      disabled={model.loading}
      onMouseEnter={model.activateDownload}
      onMouseLeave={model.deactivateDownload}
      onFocus={model.activateDownload}
      onBlur={model.deactivateDownload}
    >
      <img
        src={getDownloadIconSrc(model.isDownloadActive)}
        alt=""
        aria-hidden="true"
