...
export default function NewsInfoDialogContent({ open, item, onClose }: NewsInfoDialogProps) {
      ...
      <div className="dialog news-info__dialog">
        <div className="dialog-head news-info__head">
          <div className="news-info__head-left">
            ...
            <div className="dialog-subtitle news-info__subtitle">{t("common.admin.news.infoDialog.readOnly")}</div>
          </div>
          <div className="news-info__head-right">
            <span className={`dialog-status ${getStatusClass(data.status)}`}>{t(data.status)}</span>
            <div className="dialog-head__actions"><CloseButton label={t("common.close")} onClose={onClose} /></div>
          </div>
        </div>
        <div className="dialog-body news-info__body"><NewsInfoSections data={data} t={t} /></div>
      </div>
      <button type="button" className="dialog-backdrop-hit news-info__backdrop-hit" aria-label={t("common.close")} onClick={onClose} />
    </div>
  );
}
