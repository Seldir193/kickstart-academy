...
export default function NewsTableListContent(props: Props) {
  ...
  const state = useNewsTableState(props.items, props.rowMode, props.selectMode, props.toggleBtnRef);
  if (!state.viewItems.length) return <NewsEmptyCard text={t("common.admin.news.table.empty")} />;
  return <NewsTableLoaded props={props} state={state} t={t} lang={i18n.language} />;
}

function NewsEmptyCard({ text }: { text: string }) {
  return <section className="card"><div className="card__empty">{text}</div></section>;
}

function NewsTableLoaded(args: LoadedProps) {
  const showSwitch = canShowSwitch(args.props.rowMode);
  return <><NewsBulkActions props={args.props} state={args.state} t={args.t} /><NewsTableSection {...args} showSwitch={showSwitch} /></>;
}
