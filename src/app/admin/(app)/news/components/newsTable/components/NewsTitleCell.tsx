...
export function NewsTitleCell({
  ...
  return (
    <div className="news-list__cell news-list__cell--title">
      <TitleText item={item} t={t} />
      <ExcerptText item={item} t={t} />
      {titleDraft(item, delta.draftTitle, t)}
      {leadDraft(item, delta.draftExcerpt, t)}
    </div>
  );
}
...
function TitleText({ item, t }: { item: NewsWithProvider; t: Translate }) {
  return (
    <div className="news-list__title">
      {clean(item.title) || t("common.emptyDash")}
    </div>
  );
}
...
function ExcerptText({ item, t }: { item: NewsWithProvider; t: Translate }) {
  ...
  return (
    <div className={`news-list__excerpt ${excerpt ? "" : "is-empty"}`}>
      {excerpt || t("common.emptyDash")}
    </div>
  );
}
...
function draftWrap(label: string, value: string) {
  return (
    <div className="news-list__draft-wrap">
      <NewsDraftLine label={label} value={value} />
    </div>
  );
}
