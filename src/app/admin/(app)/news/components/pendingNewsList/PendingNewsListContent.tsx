...
export default function PendingNewsListContent(props: PendingNewsListProps) {
  ...
  if (!props.items.length) {
    return <section className="card"><div className="card__empty">
  ...
  return (
    <section className="card">
      <div className="card__body pending-news">
        {props.items.map((news) => <PendingNewsRow
          key={pickFirst(news._id, news.slug)} news={news} loading={props.loading}
          lang={i18n.language} t={t} onApprove={props.onApprove}
          onOpen={props.onOpen} onAskReject={props.onAskReject} />)}
      </div>
    </section>
  );
}
