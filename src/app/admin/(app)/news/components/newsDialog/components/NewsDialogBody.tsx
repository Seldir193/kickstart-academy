...
export default function NewsDialogBody({ state, t }: DialogComponentProps) {
  return <div className="dialog-body news-dialog__body"><NewsDialogError state={state} /><NewsDialogGrid state={state} t={t} /></div>;
}

function NewsDialogError({ state }: Pick<DialogComponentProps, "state">) {
  return state.error ? <div className="error news-dialog__error">{state.error}</div> : null;
}
