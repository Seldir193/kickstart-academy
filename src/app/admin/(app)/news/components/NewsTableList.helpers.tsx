...
function minePendingLabel(
  n: any,
  t: (key: string, options?: Record<string, unknown>) => string,
) {
  return isUpdateReview(n)
    ? t("common.admin.news.table.underReview")
    : t("common.admin.news.table.awaitingApproval");
}
...
export function statusParts(
  ...
  if (rowMode === "mine_rejected" || rowMode === "provider_rejected") {
    return {
      main: t("common.admin.news.table.rejected"),
      sub: "",
      hint: "",
      changeAt: "",
    };
  ...
  if (rowMode === "mine_pending") {
    return { main: minePendingLabel(n, t), sub: "", hint: "", changeAt: "" };
  }
  ...
  if (isApproved(n)) {
    return {
      ...
      sub: isPublished(n)
        ? t("common.admin.news.table.online")
        : t("common.admin.news.table.offline"),
      hint: "",
      changeAt: "",
    };
  }
  ...
  if (isSubmitted(n)) {
    return {
      main: t("common.admin.news.table.awaitingApproval"),
      sub: "",
      hint: "",
      changeAt: "",
    };
...
export function buildOpenAction(
  ...
  return {
    key: "open",
    icon: "/icons/edit.svg",
    title: t("common.admin.news.table.edit"),
    disabled: busy,
    run: () => onOpen(n),
  };
}
...
export function buildInfoAction(
  ...
  return {
    key: "info",
    icon: "/icons/info.svg",
    title: t("common.admin.news.table.info"),
    disabled: busy,
    run: () => onInfo(n),
  };
}
...
export function buildRejectAction(
  ...
  return {
    ...
    title: t("common.admin.news.table.reject"),
    left: true,
    disabled: busy,
    run: () => onAskReject?.(n),
  };
...
export function buildDeleteAction(
  ...
  return {
    ...
    title: t("common.admin.news.table.delete"),
    disabled: busy,
    run: () => onDeleteOne?.(n),
  };
}
...
function submitBase(
  ...
  return {
    ok,
    disabled: busy || !ok,
    tip: ok ? undefined : t("common.admin.news.table.updateFirst"),
  };
}
...
export function buildResubmitAction(
  ...
  const base = submitBase(n, busy, t);
  return {
    key: "resubmit",
    icon: "/icons/arrow_right_alt.svg",
    title: t("common.admin.news.table.resubmit"),
    left: true,
...
export function buildSubmitForReviewAction(
  ...
  return {
    key: "submit",
    icon: "/icons/arrow_right_alt.svg",
    title: t("common.admin.news.table.submitForReview"),
    left: true,
    disabled: base.disabled,
    tip: base.tip,
...
export function actionsFor(args: {
  ...
  if (rowMode === "mine_pending") {
    const a: Action[] = [buildOpenAction(n, busy, onOpen, t)];
    if (onDeleteOne) a.push(buildDeleteAction(n, busy, t, onDeleteOne));
    return a;
  }

  if (rowMode === "mine_approved") {
    const a: Action[] = [
      buildInfoAction(n, busy, onInfo, t),
      buildOpenAction(n, busy, onOpen, t),
    ];
    if (onSubmitForReview)
      a.push(buildSubmitForReviewAction(n, busy, t, onSubmitForReview));
    if (onDeleteOne) a.push(buildDeleteAction(n, busy, t, onDeleteOne));
    return a;
  }
  ...
  if (rowMode === "mine_rejected") {
    ...
    if (onResubmit) a.push(buildResubmitAction(n, busy, t, onResubmit));
    if (onDeleteOne) a.push(buildDeleteAction(n, busy, t, onDeleteOne));
    return a;
  }
  ...
  if (rowMode === "provider_approved") {
    ...
    if (onAskReject) a.push(buildRejectAction(n, busy, t, onAskReject));
    if (onDeleteOne) a.push(buildDeleteAction(n, busy, t, onDeleteOne));
    return a;
  }
  ...
  return [buildOpenAction(n, busy, onOpen, t)];
}
