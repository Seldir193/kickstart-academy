...
// async function fetchWith429Retry(url: string, init: RequestInit, tries = 3) {
//   let attempt = 0;
...
//   const p = runQueued(async () => {
//     try {
//       const res = await fetchWith429Retry(`${NEWS_API}?${qs.toString()}`, {
//         signal: args.signal,
//       });

//       const js = await readJson<ListResponse>(res);
//       if (!res.ok || !js?.ok)
//         throw new Error(js?.error || "Could not load news.");
//       return js;
//     } finally {
//       inFlight.delete(key);
//     }
...
//     const wait = retryAfterMs(res) || 350 + i * 500;
//     lastErr = new Error("Too Many Requests");
...
//   const js = await readJson<ListResponse>(res);
//   if (!res.ok || !js?.ok) throw new Error(js?.error || "Could not load news.");
//   return js;
// }
...
// //   const res = await fetch(`${NEWS_API}?${qs.toString()}`, {
// //     signal: args.signal,
// //   });
// //   const js = await readJson<ListResponse>(res);
// //   if (!res.ok || !js?.ok) throw new Error(js?.error || "Could not load news.");
