...
export default function CoachEditDialogMount({ model }: { model: CoachPageModel }) {
  return <CoachDialog open={!!model.dialogs.editItem} mode="edit" initial={model.dialogs.editItem || undefined} onClose={() => model.dialogs.setEditItem(null)} onSubmit={(values) => submitEdit(model, values)} />;
}

async function submitEdit(model: CoachPageModel, values: Partial<Coach>) {
  if (!model.dialogs.editItem) return;
  const slug = cleanStr((model.dialogs.editItem as any).slug);
