...
export default function CoachInfoDialogsMount({ model }: { model: CoachPageModel }) {
  return (
    <>
      <CoachInfoDialog open={model.dialogs.rejectInfoOpen} coach={model.dialogs.rejectInfoTarget} onClose={() => closeRejectInfo(model)} />
      <CoachPublishedInfoDialog open={model.dialogs.publishedInfoOpen} coach={model.dialogs.publishedInfoTarget} onClose={() => closePublishedInfo(model)} />
    </>
  );
}
