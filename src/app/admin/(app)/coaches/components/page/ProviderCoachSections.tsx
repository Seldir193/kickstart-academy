...
export default function ProviderCoachSections({
  model,
}: {
  ...
  return (
    <>
      <ApprovedProvidersSection
        model={model}
        title={t("common.admin.coaches.page.providersApproved")}
      />
      <RejectedProvidersSection
        model={model}
        title={t("common.admin.coaches.page.providersRejected")}
      />
    </>
  );
}
...
function ApprovedProvidersSection({ model, title }: SectionProps) {
  return (
    <CoachTableSection
      title={title}
      count={model.state.providersApprovedAll.length}
      page={model.state.providersApproved.page}
      pages={model.state.providersApproved.pages}
      onPrev={() => prevApproved(model)}
      onNext={() => nextApproved(model)}
      tableProps={approvedProps(model)}
    />
  );
}
...
function RejectedProvidersSection({ model, title }: SectionProps) {
  return (
    <CoachTableSection
      title={title}
      count={model.state.providersRejectedAll.length}
      page={model.state.providersRejected.page}
      pages={model.state.providersRejected.pages}
      onPrev={() => prevRejected(model)}
      onNext={() => nextRejected(model)}
...
function approvedProps(model: CoachPageModel) {
  return {
    items: model.state.providersApproved.slice,
    selectMode: model.state.selects.selectApprovedProviders,
    onToggleSelectMode: () =>
      model.state.selects.setSelectApprovedProviders(
        (selected: boolean) => !selected,
      ),
    busy: model.muts.mutating,
    onOpen: model.dialogs.setEditItem,
    onDeleteMany: model.muts.deleteMany,
    toggleBtnRef: model.refs.approvedProvidersToggleRef,
    onInfo: (coach: Coach) => openPublishedInfo(model, coach),
    onUnapprove: model.openDelete,
    onReject: (coach: Coach) => openReject(model, coach),
...
function rejectedProps(model: CoachPageModel) {
  return {
    items: model.state.providersRejected.slice,
    selectMode: model.state.selects.selectRejectedProviders,
    onToggleSelectMode: () =>
      model.state.selects.setSelectRejectedProviders(
        (selected: boolean) => !selected,
      ),
    busy: model.muts.mutating,
    onOpen: model.dialogs.setEditItem,
    onDeleteMany: model.muts.deleteMany,
    toggleBtnRef: model.refs.rejectedProvidersToggleRef,
...
function prevApproved(model: CoachPageModel) {
  model.state.pages.setPageApprovedProviders((page: number) =>
    Math.max(1, page - 1),
  );
}
...
function nextApproved(model: CoachPageModel) {
  model.state.pages.setPageApprovedProviders((page: number) =>
    Math.min(model.state.providersApproved.pages, page + 1),
  );
}

function prevRejected(model: CoachPageModel) {
  model.state.pages.setPageRejectedProviders((page: number) =>
    Math.max(1, page - 1),
  );
}
...
function nextRejected(model: CoachPageModel) {
  model.state.pages.setPageRejectedProviders((page: number) =>
    Math.min(model.state.providersRejected.pages, page + 1),
  );
}
