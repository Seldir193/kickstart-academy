...
export default function CoachCreateDialogMount({
  model,
}: {
  ...
  return (
    <CoachDialog
      open={model.dialogs.createOpen}
      mode="create"
      onClose={() => model.dialogs.setCreateOpen(false)}
      onSubmit={(values) => submitCreate(model, values)}
    />
  );
}
