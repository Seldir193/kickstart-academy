...
export function useFranchiseLocationSelection() {
  const [minePendingSelectMode, setMinePending, toggleMinePendingSelectMode] =
    useToggleState();
  const [
  ...
  const resetSelection = useCallback(() => {
    setMinePending(false);
    setMineApproved(false);
    setMineRejected(false);
    setProvApproved(false);
    setProvRejected(false);
  }, [
    setMinePending,
    setMineApproved,
    setMineRejected,
