...
function Hint({ show, hint }: { show: boolean; hint: string }) {
  ...
  return <div className="news-list__draft-wrap"><span className="news-list__draft-label">{hint}</span></div>;
}

export default function LocationTitleCell({ item, showHint, hint }: Props) {
  const addr = clean((item as any).address);
  return (
    <div className="news-list__cell news-list__cell--title">
      ...
      <div className={`news-list__excerpt ${addr ? "" : "is-empty"}`}>{addr || "—"}</div>
    </div>
  );
}
