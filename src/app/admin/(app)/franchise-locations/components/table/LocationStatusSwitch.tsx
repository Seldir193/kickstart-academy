...
export default function LocationStatusSwitch(p: Props) {
  ...
  return (
    <span className={wrapClass(p.busy, p.disabled)} onClick={stop} onMouseDown={stop} onPointerDown={stop}>
      <LocationSwitch checked={p.published} busy={p.busy} disabled={p.disabled} onToggle={() => toggle(p)} />
    </span>
  );
}
