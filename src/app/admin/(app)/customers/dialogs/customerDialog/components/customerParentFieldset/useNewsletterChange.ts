...
function applyUpdatedCustomer(p: CustomerParentFieldsetProps, updated: Customer, next: boolean) {
  p.setForm((previous: Customer) => ({ ...previous, ...updated, newsletter: (updated as any)?.newsletter ?? next }));
}
...
function showUpdateError(p: CustomerParentFieldsetProps, t: TFunction, error: unknown) {
  const key = "common.admin.customers.customerDialog.errors.newsletterUpdateFailed";
  setTemporaryError(p, toastErrorMessage(t, error, key));
}

export async function changeNewsletter(p: CustomerParentFieldsetProps, next: boolean, t: TFunction) {
  p.setErr(null);
  ...
  try { applyUpdatedCustomer(p, await saveNewsletter(p, next), next); }
  catch (error) { setNewsletterValue(p, previous); showUpdateError(p, t, error); }
  finally { p.setNewsletterBusy(false); }
}
