...
function isCampOffer(offer: Offer | null) {
  ...
  return type === "camp" || title.includes("camp");
}

function isPowertrainingOffer(offer: Offer | null) {
  const type = safeText((offer as any)?.type).toLowerCase();
  const title = safeText((offer as any)?.title).toLowerCase();
  return type.includes("powertraining") || title.includes("powertraining");
}
...
export default function BookDialog({
  ...
  const isCamp = useMemo(() => isCampOffer(selectedOffer), [selectedOffer]);
  const isPowertraining = useMemo(() => {
...
// function memberSelfLabel(member: FamilyMember | null) {
//   if (!member) return "Bitte auswählen …";
//   return (
//     `${member.parent.firstName} ${member.parent.lastName}`.trim() ||
//     "Kunde selbst"
//   );
// }
...
// //     const children = Array.isArray(member.children) ? member.children : [];
// //     return children
// //       .filter((child) => safeText(child?.uid))
// //       .map((child) => ({
// //         uid: safeText(child.uid),
// //         label: childLabel(child),
// //         parentId: member._id,
...
// //       .filter((child) => {
// //         const uid = safeText(child?.uid);
// //         const firstName = safeText(child?.firstName);
// //         const lastName = safeText(child?.lastName);
// //         const fullName = `${firstName} ${lastName}`.trim().toLowerCase();

// //         if (!uid) return false;
// //         if (!firstName && !lastName) return false;
// //         if (fullName === "kunde selbst") return false;
...
//   // useDropdownOutsideClose([
//   //   { ref: memberDropdownRef, close: () => setIsMemberDropdownOpen(false) },
//   //   { ref: courseDropdownRef, close: () => setIsCourseDropdownOpen(false) },
//   //   { ref: offerDropdownRef, close: () => setIsOfferDropdownOpen(false) },
//   // ]);

//     useDropdownOutsideClose([
//     { ref: memberDropdownRef, close: () => setIsParentDropdownOpen(false) },
//     { ref: childDropdownRef, close: () => setIsChildDropdownOpen(false) },
//     { ref: courseDropdownRef, close: () => setIsCourseDropdownOpen(false) },
//     { ref: offerDropdownRef, close: () => setIsOfferDropdownOpen(false) },
//   ]);
...
//   //   if (baseSelectedId) return baseSelectedId;
//   //   return family?.[0]?._id || "";
//   // }, [selectedMemberId, baseSelectedId, family]);
...
//   //   if (bookingTarget === "child" && selectedChildOption) {
//   //     return (
//   //       family.find((member) => member._id === selectedChildOption.parentId) ||
//   //       null
//   //     );
//   //   }

//   //   return family.find((member) => member._id === selfMemberId) || family[0];
//   // }, [family, bookingTarget, selectedChildOption, selfMemberId]);
...
//   //   if (!rawMember) return null;
//   //   if (parentIdx < 0) return rawMember;
...
//   //   const selectedParent = parentList[parentIdx] || rawMember.parent;

//   //   return {
//   //     ...rawMember,
//   //     parent: {
//   //       salutation: safeText(selectedParent?.salutation),
//   //       firstName: safeText(selectedParent?.firstName),
//   //       lastName: safeText(selectedParent?.lastName),
//   //       email: safeText(selectedParent?.email),
//   //       phone: safeText((selectedParent as any)?.phone),
//   //       phone2: safeText((selectedParent as any)?.phone2),
//   //     },
//   //   };
//   // }, [family, bookingTarget, selectedChildOption, selfMemberId]);

//     const selectedParent: FamilyMember | null = useMemo(() => {
//     if (!family?.length) return null;
...
//     if (bookingTarget === "self") return memberSelfLabel(selectedMember);
//     if (activeChild) return childLabel(activeChild);
//     return "Bitte Kind auswählen …";
//   }, [bookingTarget, selectedMember, activeChild]);
...
//     const selectedParent = parentPayloadFromMember(selectedMember);
...
// function pickChildByUid(member: FamilyMember | null, uid: string) {
//   const u = safeText(uid);
//   if (!member || !u) return null;

//   const c: any = member.child;
//   if (safeText(c?.uid) === u) return member.child as any;

//   const arr = Array.isArray(member.children) ? member.children : [];
//   const hit = arr.find((x: any) => safeText(x?.uid) === u);
//   return hit || null;
// }
...
//   const selectedMember: FamilyMember | null = useMemo(() => {
//     if (!family?.length) return null;
//     return family.find((m) => m._id === memberId) || family[0] || null;
//   }, [family, memberId]);

//   const memberChild: FamilyChild | null = useMemo(() => {
//     const byUid = pickChildByUid(selectedMember, safeText(initialChildUid));
//     if (byUid) return byUid;
//     return firstChildOf(selectedMember);
//   }, [selectedMember, initialChildUid]);

//   const activeChild: FamilyChild | null = useMemo(() => {
//     if (bookingTarget !== "child") return null;
//     return memberChild;
//   }, [bookingTarget, memberChild]);
...
//   const selectedMemberLabel = useMemo(() => {
//     if (!selectedMember) return "Bitte auswählen …";
...
//     const uid = safeText((activeChild as any)?.uid);
//     const childPayload =
//       bookingTarget === "child"
//         ? {
//             uid,
...
//       const freshRaw = await fetchCustomerById(customerId);
//       const fresh = freshRaw ? applySelectedChildToFresh(freshRaw, uid) : null;
