...
export function applySelectedChildToFresh(fresh: any, uid: string) {
  ...
  return selectedFreshChild(fresh, normalizedUid)
    ? withFreshChild(fresh, selectedFreshChild(fresh, normalizedUid))
    : clearFreshChild(fresh);
}
...
function selectedFreshChild(fresh: any, uid: string) {
  ...
  return (
    children.find((child: any) => safeText(child?.uid) === uid) ||
    (safeText(legacy?.uid) === uid ? legacy : null)
  );
}
