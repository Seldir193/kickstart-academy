...
export function useBookFamilyScope(
  customerId: string,
  initialChildUid: string | undefined,
  t: TFunc,
): BookFamilyScope {
...
function useInitialParentSelection(
  ...
  useEffect(() => {
    ...
    selection.setSelectedParentId(
      parentOptions[0].id ||
        (baseSelectedId ? `${baseSelectedId}::parent::0` : ""),
    );
...
function useInitialChildSelection(
  ...
  useEffect(() => {
    ...
    selection.bookingTarget,
    selection.selectedChildUid,
    initialChildUid,
    childOptions,
  ]);
}
...
function buildFamilyScope(
  ...
  const selfMemberId = selfMemberIdFor(
    selection.selectedParentId,
    base.baseSelectedId,
    base.family,
  );
  const selectedParent = selectedParentFromFamily(base.family, selfMemberId);
  const selectedChildOption = selectedChildOptionFor(
    options.childOptions,
    selection.selectedChildUid,
  );
  return {
    family: base.family,
    familyLoading: base.familyLoading,
    familyError: base.familyError,
    ...options,
    selectedParent,
    selectedParentLabel: selectedParentLabel(selectedParent, t),
    selfMemberId,
    activeChild: activeChild(selection.bookingTarget, selectedChildOption),
    ...selection,
  };
}

function selectedChildOptionFor(
  childOptions: ChildOption[],
...
function activeChild(
  ...
  selectedChildOption: ChildOption | null,
) {
