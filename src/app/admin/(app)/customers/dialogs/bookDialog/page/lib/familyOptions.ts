...
export function selectedParentFromFamily(
  ...
  return member
    ? withSelectedParent(member, parentIndexFromMemberId(selfMemberId))
    : null;
}
...
export function selfMemberIdFor(
...
function childOptionsForMember(member: FamilyMember, t: TFunc) {
  ...
  return children
    .filter((child) => isValidChild(child, member))
...
function isValidChild(child: FamilyChild, member: FamilyMember) {
  ...
  if (names.full === "kunde selbst" || names.full.includes("kunde selbst"))
    return false;
  if (names.full.includes("eltern") || names.full.includes("parent"))
    return false;
...
function parentOption(
  ...
  return {
    id: parentOptionId(memberId, idx),
    label:
...
function selectedRawMember(
  ...
  return (
    members.find((member) => member._id === baseId) ||
    members.find((member) => member._id === selfMemberId) ||
    members[0] ||
    null
  );
}

function withSelectedParent(
  member: FamilyMember,
  parentIdx: number,
): FamilyMember {
