  ...
  if (!props.scope.familyLoading)
    return (
      <div className="dialog-value">
        {props.t("common.admin.customers.cancelDialog.currentCustomer")}
      </div>
    );
  return null;
}
...
function ChildSelect({ t, scope }: Props) {
  const value = scope.activeChild
    ? childName(
        scope.activeChild,
        t("common.admin.customers.cancelDialog.selectChild"),
      )
    : t("common.admin.customers.cancelDialog.selectChild");
  return (
    <SelectField
      label={t("common.admin.customers.cancelDialog.child")}
...
function SelectBox(props: SelectFieldProps) {
  return (
    <div
      className={"ks-selectbox" + (props.open ? " ks-selectbox--open" : "")}
      ref={props.rootRef}
    >
...
function SelectPanel(props: SelectFieldProps) {
  return (
    <div className="ks-selectbox__panel" role="listbox">
      {props.items.map((item) => renderSelectOption(item, props))}
    </div>
  );
}
...
function optionClass(id: string, activeId: string) {
  return (
    ...
    (id === activeId ? " ks-selectbox__option--active" : "")
  );
}
...
function ScopeButtons({ t, scope }: Props) {
  return (
    <div className="cancel-dialog__scopeButtons">
      <ScopeButton t={t} scope={scope} target="self" />
      <ScopeButton t={t} scope={scope} target="child" />
    </div>
  );
}
...
function ScopeButton({ t, scope, target }: Props & { target: BookingTarget }) {
  return (
    <button
      type="button"
      className={scopeButtonClass(scope.bookingTarget, target)}
      onMouseDown={rememberButtonFocusState}
      onClick={(e: React.MouseEvent<HTMLButtonElement>) =>
        toggleButtonFocus(e, () => scope.setBookingTarget(target))
      }
    >
      {scopeButtonLabel(t, target)}
    </button>
  );
}
...
function scopeButtonClass(activeTarget: BookingTarget, target: BookingTarget) {
  return (
    ...
    (activeTarget === target ? " cancel-dialog__scopeBtn--active" : "")
  );
}
...
function ScopeSummary({ t, scope }: Props) {
  return (
    <div className="cancel-dialog__summary">
      <SummaryItem
        label={t("common.admin.customers.cancelDialog.parent")}
        value={summaryParentName(t, scope)}
      />
      <SummaryItem
        label={t("common.admin.customers.cancelDialog.child")}
        value={summaryChildName(t, scope)}
      />
    </div>
  );
}
...
function summaryParentName(t: TFunc, scope: FamilyScopeState) {
  if (!scope.selectedParent)
    return t("common.admin.customers.cancelDialog.empty");
...
function summaryChildName(t: TFunc, scope: FamilyScopeState) {
  return scope.bookingTarget === "child" && scope.activeChild
    ? childName(
        scope.activeChild,
