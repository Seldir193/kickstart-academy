...
function CancelDateField({ t, bookings, submit }: Props) {
  return (
    <div>
      <DateField
        label={t("common.admin.customers.cancelDialog.receiptDateRequired")}
        value={submit.cancelDate}
        onChange={(nextIso) => changeCancelDate(nextIso, submit)}
        disabled={dateDisabled(bookings, submit)}
...
function EndDateField({ t, bookings, submit }: Props) {
  return (
    <div>
      <DateField
        label={t("common.admin.customers.cancelDialog.endDateRequired")}
        value={submit.endDate}
        onChange={(nextIso) => changeEndDate(nextIso, submit)}
        disabled={dateDisabled(bookings, submit)}
        placeholder={t("common.placeholders.date")}
      />
      {submit.endBeforeStart && (
        <div className="cancel-dialog__error mt-1">
          {t("common.admin.customers.cancelDialog.endDateAfterReceipt")}
        </div>
      )}
    </div>
  );
...
function dateDisabled(
  ...
  return (
    !bookings.selected ||
    bookings.selectedIsCancelled ||
    submit.disabledByNonCancelableCourse
  );
}
    ...
    <div>
      ...
      <textarea
        ...
        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
          submit.setReason(e.target.value)
        }
        placeholder={t("common.admin.customers.cancelDialog.reasonPlaceholder")}
        disabled={dateDisabled(bookings, submit)}
      />
    </div>
  );
...
function DetailNotes({ t, bookings, submit }: Props) {
  return (
    <>
      {bookings.selected && bookings.selectedIsCancelled && (
      ...
      {submit.disabledByNonCancelableCourse && (
        <div className="cancel-dialog__note">
          {t("common.admin.customers.cancelDialog.cancellationsNotAllowed")}
        </div>
      )}
    </>
  );
