...
export function useCancelSubmit(
  ...
  const submit = () =>
    submitCancel(customer, bookings, state, onClose, onChanged, t);
...
function submitDerived(
  state: ReturnType<typeof useSubmitFields>,
  bookings: CancelBookingsState,
) {
  const endBeforeStart = Boolean(
    state.endDate && state.cancelDate && state.endDate < state.cancelDate,
  );
...
async function postCancelWithHandling(
  customer: Customer,
  bookings: CancelBookingsState,
  state: ReturnType<typeof useSubmitFields>,
  onClose: () => void,
