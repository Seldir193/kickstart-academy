...
export function useStornoSubmit(
  ...
  const submit = () => submitStorno(customer, bookings, state, onClose, onChanged, t);
...
async function submitStorno(customer: Customer, bookings: StornoBookingsState, state: ReturnType<typeof useSubmitFields>, onClose: () => void, onChanged: (freshCustomer: Customer) => void, t: TFunc) {
  if (!canSubmit(customer, bookings)) return;
  state.setSaving(true);
  bookings.setErr(null);
  await postStornoWithHandling(customer, bookings, state, onClose, onChanged, t);
}
...
async function postStornoWithHandling(customer: Customer, bookings: StornoBookingsState, state: ReturnType<typeof useSubmitFields>, onClose: () => void, onChanged: (freshCustomer: Customer) => void, t: TFunc) {
  try {
    ...
    bookings.setErr(toastErrorMessage(t, e, "common.admin.customers.stornoDialog.errors.cancelFailed"));
  } finally {
    state.setSaving(false);
  }
}

async function refreshAfterStorno(customer: Customer, onChanged: (freshCustomer: Customer) => void) {
