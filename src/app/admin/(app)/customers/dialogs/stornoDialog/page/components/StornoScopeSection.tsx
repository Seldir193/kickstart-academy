...
function ScopeContent(props: Props) {
  if (props.scope.family && props.scope.family.length > 0)
    return <FamilyControls {...props} />;
  if (!props.scope.familyLoading)
    return (
      <div className="dialog-value">
        {props.t("common.admin.customers.stornoDialog.currentCustomerInfo")}
      </div>
    );
  return null;
}
...
function ParentSelect({ t, scope }: Props) {
  return (
    <SelectField
      label={t("common.admin.customers.stornoDialog.parent")}
      open={scope.isParentDropdownOpen}
      setOpen={scope.setIsParentDropdownOpen}
      rootRef={scope.parentDropdownRef}
      value={
        scope.selectedParentLabel ||
        t("common.admin.customers.stornoDialog.selectParent")
      }
      items={scope.parentOptions}
      activeId={scope.selfMemberId}
...
function ChildSelect({ t, scope }: Props) {
  const value = activeChildLabel(
    scope,
    t("common.admin.customers.stornoDialog.selectChild"),
  );
  return (
    <SelectField
      label={t("common.admin.customers.stornoDialog.child")}
      open={scope.isChildDropdownOpen}
      setOpen={scope.setIsChildDropdownOpen}
      rootRef={scope.childDropdownRef}
      value={value}
      items={scope.childOptions.map((item) => ({
        id: item.uid,
        label: item.label,
      }))}
      activeId={scope.selectedChildUid}
      onSelect={scope.setSelectedChildUid}
    />
  );
}
...
function SelectTrigger(props: SelectFieldProps) {
  return (
    ...
    >
      <span className="ks-selectbox__label">{props.value}</span>
  ...
  return (
    <div className="ks-selectbox__panel" role="listbox">
      {props.items.map((item) => (
        <SelectOption key={item.id} item={item} props={props} />
      ))}
    </div>
  );
}
...
function SelectOption({
  ...
  return (
    <button
      type="button"
      className={optionClass(item.id === props.activeId)}
      onClick={() => selectOption(item.id, props)}
    >
      {item.label}
    </button>
  );
...
function ScopeButtons({ t, scope }: Props) {
  return (
    <div className="storno-dialog__scopeButtons">
      <ScopeButton t={t} scope={scope} value="self" />
      <ScopeButton t={t} scope={scope} value="child" />
    </div>
  );
}
...
function ScopeButton({ t, scope, value }: Props & { value: BookingTarget }) {
  return (
    <button
      type="button"
      className={scopeButtonClass(scope.bookingTarget === value)}
      onMouseDown={rememberButtonFocusState}
      onClick={(e) => toggleButtonFocus(e, () => scope.setBookingTarget(value))}
    >
      {scopeButtonLabel(t, value)}
    </button>
  );
}
...
function scopeButtonClass(active: boolean) {
  return (
    ...
    (active ? " storno-dialog__scopeBtn--active" : "")
  );
}
...
function scopeButtonLabel(t: TFunc, value: BookingTarget) {
  return t(
    value === "self"
      ? "common.admin.customers.stornoDialog.customerSelf"
      : "common.admin.customers.stornoDialog.child",
  );
}
...
function ScopeSummary({ t, scope }: Props) {
  return (
    <div className="storno-dialog__summary">
      <SummaryItem
        label={t("common.admin.customers.stornoDialog.parent")}
        value={parentSummary(scope)}
      />
      <SummaryItem
        label={t("common.admin.customers.stornoDialog.child")}
        value={childSummary(scope)}
      />
    </div>
  );
}
...
function childSummary(scope: FamilyScopeState) {
  ...
  return (
    `${scope.activeChild.firstName} ${scope.activeChild.lastName}`.trim() || "—"
  );
}
