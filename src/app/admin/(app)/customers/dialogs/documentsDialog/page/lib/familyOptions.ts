...
export function buildChildOptions(family: FamilyMember[] | null, t: (key: string) => string) {
  return familyMembers(family).flatMap((member) => childOptionsForMember(member, t));
}

export function buildParentOptions(family: FamilyMember[] | null, t: (key: string) => string) {
  return familyMembers(family).flatMap((member) => parentOptionsForMember(member, t));
}
...
function childOptionsForMember(member: FamilyMember, t: (key: string) => string) {
  const parent = parentIdentity(member);
  return memberChildren(member).filter((child) => isValidChild(child, parent)).map((child) => childOption(member, child, t));
}
...
function childValidationData(child: FamilyChild) {
  ...
  return { uid: safeText(child?.uid), first, last, full: `${first.toLowerCase()} ${last.toLowerCase()}`.trim() };
}

function isParentLikeChild(data: ReturnType<typeof childValidationData>, parent: ParentIdentity) {
  ...
  if (data.first.toLowerCase() === parent.first && data.last.toLowerCase() === parent.last) return true;
  return data.full === parent.full;
}

function childOption(member: FamilyMember, child: FamilyChild, t: (key: string) => string): ChildOption {
...
function childLabel(child: FamilyChild, t: (key: string) => string) {
  return `${child.firstName} ${child.lastName}`.trim() || t("admin.customers.documents.child.fallback");
}

function parentOptionsForMember(member: FamilyMember, t: (key: string) => string) {
  return rawParents(member).map((parent, idx) => parentOption(member, parent, idx, t));
}
...
function parentOption(member: FamilyMember, parent: any, idx: number, t: (key: string) => string): ParentOption {
  return { id: parentOptionId(member._id, idx), label: parentLabel(parent, t) };
}

function parentLabel(parent: any, t: (key: string) => string) {
  return `${safeText(parent?.firstName)} ${safeText(parent?.lastName)}`.trim() || t("admin.customers.documents.parent.fallback");
}
