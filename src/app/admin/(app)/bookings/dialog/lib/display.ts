...
export function inferVenue(booking: Booking, t: Translate) {
  ...
  return (
    venueFromProgram(booking.message) ||
    extractAddress(booking.message, t) ||
    "—"
  );
}
...
export function adminMessageLines(
  booking: Booking,
...
function extractAddress(msg: string | undefined, t: Translate) {
  return messageToLines(msg, t).reduce(
    (found, ln) => found || addressValue(ln),
    "",
  );
}
...
function isWeeklyText(text: string) {
  return /foerdertraining|kindergarten|torwarttraining|foerdertraining_athletik/.test(
    text,
  );
}

function isOneTimeText(text: string) {
  return /personaltraining|personal training|einzeltraining|1:1|1to1|individual|coach education|coacheducation|rent-a-coach|rentacoach|clubprogram|club program/.test(
    text,
  );
}
...
function adminMessageParts(
  ...
  return [
    registrationLine(booking, t),
    childMessageLine(booking, detail, t),
    birthLine(detail, t, lang),
    contactLine(detail, t),
    addressLine(booking, detail, t),
    phoneLine(detail, t),
  ];
}
...
function childMessageLine(
  ...
  return child
    ? `${t("common.admin.bookings.dialog.message.child")}: ${child}`
    : "";
}
...
function phoneLine(detail: BookingDetail, t: Translate) {
  ...
  return phone
    ? `${t("common.admin.bookings.dialog.message.phone")}: ${phone}`
    : "";
}
