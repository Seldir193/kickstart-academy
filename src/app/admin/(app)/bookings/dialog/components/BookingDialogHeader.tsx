    ...
    <div className="dialog-head booking-dialog__head">
      <div className="booking-dialog__head-main">
        <div className="booking-dialog__title-row">
          ...
          <StatusBadge model={model} t={t} />
          <PaymentBadge booking={booking} t={t} />
        </div>
      </div>
      <HeaderActions onClose={onClose} t={t} />
    </div>
  );
}
...
function StatusBadge({
  ...
  return (
    <span className={statusBadgeClass(model.status)}>
      {t(`common.admin.bookings.status.${asStatus(model.status)}`)}
    </span>
  );
}
...
function PaymentBadge({ booking, t }: Pick<Props, "booking" | "t">) {
  ...
  return (
    <span className={paymentBadgeClass(booking.paymentStatus)}>
      {t(`common.admin.bookings.payment.${booking.paymentStatus}`)}
    </span>
  );
}
