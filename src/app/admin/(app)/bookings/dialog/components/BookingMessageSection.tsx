  ...
  return (
    <section className="dialog-section booking-dialog__section">
      ...
      <div className="dialog-section__body">
        <MessageContent lines={model.messageLines} t={t} />
      </div>
    </section>
  );
}
  ...
  return (
    <div className="booking-dialog__message-list">
      {lines.map((line, index) => (
        <MessageRow key={`${line.raw}-${index}`} line={line} t={t} />
      ))}
    </div>
  );
}
...
function MessageRow({ line, t }: { line: MessageLine; t: Translate }) {
  if (!line.label)
    return (
      <div className="booking-dialog__message-line">
        <div className="dialog-value">{line.raw}</div>
      </div>
    );
  return (
    <div className="booking-dialog__message-row">
      <div className="dialog-label">{line.label}</div>
      <div className="dialog-value">
        {line.value || t("common.admin.bookings.dialog.empty")}
      </div>
    </div>
  );
}
