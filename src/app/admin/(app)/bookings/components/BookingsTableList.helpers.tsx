//src\app\admin\(app)\bookings\components\BookingsTableList.helpers.tsx
"use client";
...
export function renderRow(
  ...
  return (
    <li
      ...
      aria-label={selectMode ? labelSelect(b) : labelOpen(b)}
    >
      ...
      <div className="news-list__cell bookings-mono">{programAbbr(b)}</div>
      <div className="news-list__cell">{asStatus(b.status)}</div>
      <div className="news-list__cell">{renderPayment(b)}</div>
      <div className="news-list__cell">{formatDateDe(b.createdAt)}</div>
...
export function focusClearOrCancel(
  ...
  if (count >= 2) requestAnimationFrame(() => clearBtnRef.current?.focus());
  if (prev >= 2 && count < 2)
