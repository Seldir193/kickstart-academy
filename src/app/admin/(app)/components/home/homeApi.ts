...
export async function fetchAdminHomeCounts(): Promise<AdminHomeCounts> {
  return {
    onlineCount: await fetchCount(
      "/api/admin/offers?onlineActive=true&limit=1",
    ),
    placesCount: await fetchCount("/api/admin/places?limit=1"),
    newsletterLeads: await fetchCount(
      "/api/admin/customers?tab=newsletter&limit=1",
    ),
    openBookingsCount: await fetchCount(
...
export async function fetchAdminName() {
  const json = await fetchJson<{ ok?: boolean; user?: Record<string, string> }>(
    "/api/admin/auth/me",
    {},
  );
