...
export default function PartnerBulkActions(props: Props) {
  if (!props.selectMode) return <SelectModeButton {...props} />;
  return <PartnerBulkBar {...props} />;
}
...
function SelectModeButton(props: Props) {
  ...
  return (
    <div className="actions news-admin__actions">
      ...
      >
        {t("admin.partners.bulk.select")}
      </button>
    </div>
  );
}

function PartnerBulkBar(props: Props) {
  const { t } = useTranslation();
...
function ClearButton(props: Props) {
  ...
  return (
    <button
      ...
      disabled={props.disabled}
    >
      {t("admin.partners.bulk.clearSelection")}
    </button>
  );
}
...
function DeactivateButton(props: Props) {
  ...
  return (
    ...
    >
      {t("admin.partners.bulk.deactivate")} ({props.count})
    </button>
  );
}
...
function DeleteButton(props: Props) {
  ...
  return (
    <button
      ...
      disabled={props.disabled || props.count === 0}
    >
      {t("admin.partners.bulk.delete")} ({props.count})
    </button>
  );
}
...
function CancelButton(props: Props) {
  ...
  return (
    <button
      ...
      disabled={props.disabled}
    >
      {t("admin.partners.bulk.cancel")}
    </button>
  );
}

function getSelectAllLabel(props: Props, t: (key: string) => string) {
  if (props.isAllSelected) return t("admin.partners.bulk.clearSelection");
  return t("admin.partners.bulk.selectAll");
}
