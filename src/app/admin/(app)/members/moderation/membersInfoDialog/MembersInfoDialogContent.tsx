...
import {
  buildMembersInfoData,
  getBadgeClass,
} from "./membersInfoDialog.helpers";
import {
  MemberAvatarSection,
  MemberProfileSection,
} from "./MembersInfoSections";
import type {
  MembersInfoData,
  MembersInfoDialogProps,
} from "./membersInfoDialog.types";

type ViewProps = {
  data: MembersInfoData;
  item: AdminMember;
  onClose: () => void;
  t: TFunction;
};
    ...
    <div className="dialog-head members-info-dialog__head">
      <div className="members-info-dialog__head-main">
        ...
        <div className="members-info-dialog__title-row">
          ...
          <span className={`members-info-dialog__badge ${getBadgeClass(item)}`}>
            {data.role}
          </span>
        </div>
  ...
  return (
    ...
    >
      <button
        ...
        aria-label={props.t("common.admin.members.info.close")}
...
export default function MembersInfoDialogContent({
  open,
  item,
  onClose,
}: MembersInfoDialogProps) {
  ...
  const data = useMemo(
    () => (item ? buildMembersInfoData(t, item) : null),
    [item, t],
  );
