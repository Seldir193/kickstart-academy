...
const DAY_LONG_BY_LOCALE: Record<string, Record<string, string>> = {
  de: {
    Mo: "Montag",
    Di: "Dienstag",
    Mi: "Mittwoch",
...
export function getDayLong(day?: string) {
  const normalized = normDay(day);
  const locale = getCurrentLocale();
  return DAY_LONG_BY_LOCALE[locale]?.[normalized] || normalized;
}

export function getOfferDayLabel(offer?: Offer | null) {
  if (offer?.day) return getDayLong(offer.day);

  const fallbackDay = Array.isArray(offer?.days) ? offer.days[0] : "";
  return fallbackDay ? getDayLong(fallbackDay) : "";
}

export function getOfferTimeLine(offer?: Offer | null) {
  if (!offer?.timeFrom || !offer?.timeTo) return "";
  return `${offer.timeFrom} – ${offer.timeTo}`;
}

export function getOfferPriceText(offer?: Offer | null) {
  if (typeof offer?.price !== "number") return "";
  return `${offer.price.toFixed(2)}€`;
}

function getEveryLabel() {
  const locale = getCurrentLocale();
  if (locale === "de") return "Jeden";
  if (locale === "tr") return "Her";
  return "Every";
}
...
export function getOfferScheduleLabel(offer?: Offer | null) {
  ...
  if (isWeeklyCourse(offer)) return `${getEveryLabel()} ${day}`;
  if (isIndividualCourse(offer)) return day;

  return "";
}

export function getOfferScheduleLine(offer?: Offer | null) {
  const schedule = getOfferScheduleLabel(offer);
  const time = getOfferTimeLine(offer);
  return [schedule, time].filter(Boolean).join(" · ");
}
